...
export async function POST(req: NextRequest) {
  const filePath = path.join(process.cwd(), "src", "utils", "tokenRecords.json");
  ...
  try {
    ...
    const newMcap = requestData.mcap;
    const newVolume = requestData.volume24h;
    const currentTime = requestData.now;

    records.previousMcap.value = newMcap;
    records.previousMcap.lastTimeUpdated = currentTime;
